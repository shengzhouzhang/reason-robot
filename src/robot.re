...
let place = (prevState: option(state), x: int, y: int, direction: direction, table: Table.table): option(state) =>
...
let move = (prevState: state, table: Table.table): state => {
...
let create = (table: Table.table): robot => { state: ref(None), table: table };